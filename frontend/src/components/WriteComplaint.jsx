...
const WriteComplaint = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const res = await fetch(`${BASE_URL}/api/complaints/submit`, {
        method: 'POST',
      ...
      await fetch(`${BASE_URL}/api/send-feedback-mail`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          email: user.email,
      ...
      <div className="w-full max-w-2xl bg-white p-6 rounded shadow space-y-6">
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="border-2 border-dashed border-gray-300 rounded py-6 text-center">
            ...
            <input type="file" name="attachments" onChange={handleChange} className="mt-2" />
          </div>
